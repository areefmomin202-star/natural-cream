...
export function ShareModal({ isOpen, onClose }: ShareModalProps) {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Natural Cream - Wake up to Gorgeous Skin (₹399 Special Offer)',
          text: 'Get 60% OFF Natural Cream night skincare for just ₹399 (MRP ₹999). Direct WhatsApp order!',
          url: currentUrl
        });
      } catch (err) {
  ...
  const shareToWhatsApp = () => {
    const text = `Hey! Check out Natural Cream - Wake up to Gorgeous Skin! It's on a 60% OFF discount for ₹399 (MRP ₹999): ${currentUrl}`;
