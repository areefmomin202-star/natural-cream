...
export function buildDynamicOrderWhatsAppMessage(productName: string, packName: string, quantity: number, totalAmount: number): string {
  return `Hi 👋 ${productName} Team,
...
export function getDynamicOrderWhatsAppUrl(
  ...
  const num = (whatsappNumber || WHATSAPP_NUMBER).replace(/\D/g, '');
  const message = buildDynamicOrderWhatsAppMessage(productName, packName, quantity, totalAmount);
