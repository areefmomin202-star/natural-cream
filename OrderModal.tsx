...
export function OrderModal({ pack, onClose }: OrderModalProps) {
  ...
  const handleSendToWhatsApp = (e: FormEvent) => {
    ...
    if (!customerName && !city && !address) {
      // Default exact requested message format
      message = pack.quantity === 1
        ? 'Hello Natural Cream, I want to order Natural Cream for ₹399. Please share the order details.'
        : `Hello Natural Cream, I want to order ${pack.name} for ₹${pack.price}. Please share the order details.`;
    } else {
      message = `Hello Natural Cream, I want to order ${pack.name} for ₹${pack.price}.\n\n` +
        `Customer Details:\n` +
  ...
  const handleInstantWhatsApp = () => {
    const url = pack.quantity === 1
      ? getWhatsAppOrderUrl('Hello Natural Cream, I want to order Natural Cream for ₹399. Please share the order details.')
      : getWhatsAppOrderUrl(`Hello Natural Cream, I want to order ${pack.name} for ₹${pack.price}. Please share the order details.`);
    window.open(url, '_blank');
    onClose();
  };
        ...
        <div className="bg-stone-900 text-white p-5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-stone-300">Official Line: {WHATSAPP_DISPLAY_NUMBER}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 bg-amber-50/70 border-b border-amber-200/60 flex items-center justify-between">
          <div>
            ...
            <div className="text-sm font-extrabold text-stone-900 mt-1">
              Natural Cream - {pack.name}
            </div>
            <div className="text-xs text-stone-500">
              MRP <del>₹{pack.mrp}</del> • {pack.discountPercent}% OFF
            </div>
          </div>
