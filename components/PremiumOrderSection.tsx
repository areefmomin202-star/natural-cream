...
import { PACK_OPTIONS, getOrderWhatsAppUrl, useProduct, getDynamicOrderWhatsAppUrl } from '../data/productData';
...
export function PremiumOrderSection({
  ...
  // Dynamic calculations strictly tied to single source of truth product.mrp
  const packUnitCount = selectedPack.quantity || 1;
  const totalAmount = selectedPack.price * quantity;
  const totalMrp = (product.mrp * packUnitCount) * quantity;
  const totalSavings = Math.max(0, totalMrp - totalAmount);
  const currentDiscountPercent = totalMrp > 0 
    ? Math.round(((totalMrp - totalAmount) / totalMrp) * 100) 
    : (selectedPack.discountPercent || product.discountPercent);

  // Exact requested WhatsApp pre-filled order URL
  const whatsappOrderUrl = getDynamicOrderWhatsAppUrl(
              ...
              >
                ...
                <div className="mt-auto pt-1 border-t border-stone-200/60 flex items-baseline justify-between w-full">
                  ...
                  <span className="text-[11px] text-stone-400 line-through">
                    ₹{(product.mrp * (pack.quantity || 1)).toLocaleString('en-IN')}
                  </span>
                </div>
              </button>
            );
