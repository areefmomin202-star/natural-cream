...
import { PACK_OPTIONS, getDynamicOrderWhatsAppUrl, useProduct } from '../data/productData';
...
export function PricingCard({ onOpenOrderModal }: PricingCardProps) {
  ...
  useEffect(() => {
    if (packs.length > 0) {
      const match = packs.find(p => p.id === selectedPack.id);
      if (match) setSelectedPack(match);
      else setSelectedPack(packs[0]);
    }
  }, [packs]);

  const selectedPackMrp = product.mrp * (selectedPack.quantity || 1);
  const selectedPackSavings = Math.max(0, selectedPackMrp - selectedPack.price);
  const selectedPackDiscount = selectedPackMrp > 0
    ? Math.round(((selectedPackMrp - selectedPack.price) / selectedPackMrp) * 100)
    : selectedPack.discountPercent;

  // Construct WhatsApp URL with exact message requested
  const getPackWhatsAppUrl = (pack: PackOption) => {
    return getDynamicOrderWhatsAppUrl(
    ...
    <div className="bg-white rounded-3xl border border-stone-200/90 shadow-sm p-5 md:p-7 relative">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-rose-600 text-white font-bold text-xs px-2 py-0.5 rounded-full shadow-xs">
                {selectedPackDiscount}% OFF
              </span>
            </div>
            <span className="text-xs font-semibold text-emerald-700">
              You Save ₹{selectedPackSavings} Today
            </span>
          </div>
        </div>
      ...
      <div className="mt-5 p-3 rounded-2xl bg-stone-50 border border-stone-200/80 flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2 text-xs text-stone-700">
          ...
          <span>
            <strong>Official WhatsApp Number:</strong> {product.whatsappDisplayNumber}
          </span>
        </div>
      </div>
